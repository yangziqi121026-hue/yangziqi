...
_MOCK_TEMPLATES = {
    ...
    "news_event": (
        ...
        "2) 事件动向：需核验\n"
        "3) 触发因素：需核验\n"
        "4) 传导逻辑：事件→相关产业→A股对应板块（占位）\n"
        "5) 受益板块：需核验\n"
        "6) 利空板块：需核验\n"
        "7) 关注方向：需核验\n"
        "8) 置信度：低\n\n"
        "> 当前为 Mock 占位。配置 DEEPSEEK_API_KEY 且 MOCK_MODE=false 后由 DeepSeek 真实研判。"
    ),
    "deepseek_short": (
        "**DeepSeek 超短线精析（占位输出）**\n\n"
        "1) 反弹逻辑：存疑——RSI 处超卖/中性区且站上 MA5/MA10，有低吸价值，但需放量确认。\n"
        "2) RSI+MACD：RSI 未超买、MACD 首次反转，属于左侧偏早信号。\n"
        "3) 最大反面风险：若大盘走弱或量能跟不上，反弹易夭折、跌破 MA10 即失败。\n"
        "4) 价位：入场/止损/目标区间基本合理，止损务必严格执行。\n"
        "5) 短线置信度：中。\n\n"
        "> 说明：当前为 Mock 模式占位。配置 DEEPSEEK_API_KEY 且 MOCK_MODE=false 后，"
        "本段将由 DeepSeek 基于真实指标生成。不构成投资建议。"
    ),
    "risk_manager": (
        "**风控结论（占位输出）**\n\n"
        "- 风险等级：中\n"
